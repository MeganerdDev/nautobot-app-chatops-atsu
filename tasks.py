...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_NAUTOBOT_CHATOPS_ATSU_xxx
...
def _is_compose_included(context, name):
    return f"docker-compose.{name}.yml" in context.nautobot_chatops_atsu.compose_files
...
def docker_compose(context, command, **kwargs):
    ...
    build_env = {
        ...
        "COMPOSE_HTTP_TIMEOUT": context.nautobot_chatops_atsu.compose_http_timeout,
        "NAUTOBOT_VER": context.nautobot_chatops_atsu.nautobot_ver,
        "PYTHON_VER": context.nautobot_chatops_atsu.python_ver,
        **kwargs.pop("env", {}),
    }
    compose_command_tokens = [
        "docker compose",
        f"--project-name {context.nautobot_chatops_atsu.project_name}",
        f'--project-directory "{context.nautobot_chatops_atsu.compose_dir}"',
    ]

    for compose_file in context.nautobot_chatops_atsu.compose_files:
        compose_file_path = os.path.join(context.nautobot_chatops_atsu.compose_dir, compose_file)
...
def run_command(context, command, service="nautobot", **kwargs):
    ...
    if is_truthy(context.nautobot_chatops_atsu.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.nautobot_chatops_atsu.python_ver}...")
...
def _get_docker_nautobot_version(context, nautobot_ver=None, python_ver=None):
    ...
    if python_ver is None:
        python_ver = context.nautobot_chatops_atsu.python_ver
    dockerfile_path = os.path.join(context.nautobot_chatops_atsu.compose_dir, "Dockerfile")
...
def lock(context, check=False, constrain_nautobot_ver=False, constrain_python_ver=False):
    ...
    if constrain_nautobot_ver:
        ...
        if constrain_python_ver:
            command += f" --python {context.nautobot_chatops_atsu.python_ver}"
        try:
            ...
            command = f"poetry add --lock git+https://github.com/nautobot/nautobot.git#{context.nautobot_chatops_atsu.nautobot_ver}"
            if constrain_python_ver:
                command += f" --python {context.nautobot_chatops_atsu.python_ver}"
            run_command(context, command)
    else:
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations nautobot_chatops_atsu"
...
@task(
    help={
        "version": "Version of Nautobot Chatops Atsu to generate the release notes for.",
    }
)
...
def pylint(context):
    ...
    command = f"{base_pylint_command} nautobot_chatops_atsu"
    if not run_command(context, command, warn=True):
        exit_code = 1

    # run the pylint_django migrations checkers on the migrations directory, if one exists
    migrations_dir = Path(__file__).absolute().parent / Path("nautobot_chatops_atsu") / Path("migrations")
    if migrations_dir.is_dir():
        migrations_pylint_command = (
            ...
            " nautobot_chatops_atsu.migrations"
        )
...
def unittest_coverage(context):
    ...
    command = "coverage report --skip-covered --include 'nautobot_chatops_atsu/*' --omit *migrations*"
...
def tests(context, failfast=False, keepdb=False, lint_only=False):
    ...
    if not is_truthy(context.nautobot_chatops_atsu.local):
